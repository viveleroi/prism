...
public class MysqlSchemaUpdater {
     ...
     */
    public boolean update_8_to_v4(StorageConfiguration storageConfig) throws SQLException {
        ...
        @Language("SQL") String updateData = "ALTER TABLE `" + storageConfig.prefix() + "activities`"
            ...
            + "ADD COLUMN `descriptor` VARCHAR(255) NULL AFTER `cause_id`;";
        ...
        for (NamedIdentity world : worldIdentityProvider.worlds()) {
            @Language("SQL") String sql = "UPDATE `" + storageConfig.prefix() + "worlds`"
                + "SET world_uuid = UNHEX(?) WHERE world = ?";

            String worldUid = TypeUtils.uuidToDbString(world.uuid());
            DB.executeInsert(sql, worldUid, world.name());
        }
