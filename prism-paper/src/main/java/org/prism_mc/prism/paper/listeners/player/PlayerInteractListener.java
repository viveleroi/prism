...
public class PlayerInteractListener extends AbstractListener implements Listener {
    ...
    public void onPlayerInteractMonitor(final PlayerInteractEvent event) {
        ...
        if (blockState instanceof ChiseledBookshelf chiseledBookshelf && event.getClickedPosition() != null) {
            ...
            var action = new PaperBlockAction(PaperActionTypeRegistry.INVENTORY_OPEN, blockState);

            var activity = PaperActivity.builder().action(action).cause(player).location(location).build();
            ...
            var action = new PaperBlockAction(PaperActionTypeRegistry.BLOCK_USE, blockState);
